...
const reportPath = path.resolve(
  import.meta.dirname,
  "../projects/kallistis-materializado-historia-v5.report.json",
);
...
const shapeFailures = [
  ...
  ...(report.output?.project?.endsWith("kallistis-materializado-historia-v5.json")
    ? []
    : ["OUTPUT_NOT_V5"]),
  ...
  JSON.stringify(
    {
      verdict: "PASS",
      scope: report.scope,
      pages: report.diagnostics.TOTAL_PAGES,
      checked: requiredZero,
      compositionFamilies: report.visualGate.compositionFamiliesUsed,
      semanticImages: {
        invalid: report.diagnostics.INVALID_IMAGE_PLACEMENTS,
        bodyMaxTextRun: report.diagnostics.BODY_MAX_CONSECUTIVE_TEXT_PAGES,
      },
    },
    null,
    2,
