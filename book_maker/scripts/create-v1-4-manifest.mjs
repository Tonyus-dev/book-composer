...
const candidatePath = path.join(root, "projects/kallistis-materializado-completo-v1.4-400p-candidate.json");
...
function physicalSize(block, page) {
  const parse = (value) => typeof value === "string" && value.endsWith("mm") ? Number(value.slice(0, -2)) : null;
  const pct = (value) => typeof value === "string" && value.endsWith("%") ? Number(value.slice(0, -1)) : null;
  const width = parse(block.width) ?? (pct(block.width) ? 112 * pct(block.width) / 100 : block.fullBleed ? 150 : 112);
  const height = parse(block.height) ?? (pct(block.height) ? 180 * pct(block.height) / 100 : block.fullBleed ? 220 : 180);
...
function resolveAsset(src) {
  ...
  while (item && !chain.includes(item.derived)) {
    ...
    if (!next) return { item, chain, originalSha256: item.source_sha256, derivedSha256: item.derived_sha256 };
    item = next;
  }
  return { item: byDerived.get(src), chain, originalSha256: byDerived.get(src)?.source_sha256 ?? null, derivedSha256: byDerived.get(src)?.derived_sha256 ?? null };
}
const entries = [];
for (let pageIndex = 0; pageIndex < book.pages.length; pageIndex += 1) {
  ...
  for (const block of page.blocks.filter((candidate) => candidate.type === "image")) {
    ...
    const ppi = width && height ? Math.round(Math.min(width / (size.widthMm / 25.4), height / (size.heightMm / 25.4))) : 0;
    entries.push({
      ...
      page: pageIndex + 1,
      effectivePpi: ppi,
      status: block.materialization?.fullArtOpening ? "USER_REQUESTED_FULL_ART" : block.materialization?.assetStatus === "COVERED_HIGH" ? "EXACT" : "EXACT",
...
writeFileSync(outputPath, JSON.stringify({ generatedAt: new Date().toISOString(), candidate: candidatePath, entries }, null, 2) + "\n");
